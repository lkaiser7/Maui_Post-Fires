...
# download ESRI csv file from ERDDAP
# https://pae-paha.pacioos.hawaii.edu/erddap/tabledap/hui_water_quality.html 
hui_data<-read_csv(paste0(huiDir, "hui_water_quality.csv")) # date/time separate
hui_data
...
# # remove first row of units
# hui_data<-hui_data[-1,]
# # add date column
# hui_data$date<-as.Date(gsub("T.*", "", hui_data$time))
# remove time column for AGOL mapping
hui_data<-hui_data[,-2]
# add short date column
hui_data$short_date<-as.Date(hui_data$date)

# data exploration
head(data.frame(hui_data))
names(hui_data)
